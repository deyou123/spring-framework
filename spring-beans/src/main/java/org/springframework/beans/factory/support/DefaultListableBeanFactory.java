...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			...
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {//判断非抽象，是单例非拉加载
				if (isFactoryBean(beanName)) {   //判断是工厂bean
					Object bean = getBean(FACTORY_BEAN_PREFIX + beanName);  //获取工厂bean
					if (bean instanceof FactoryBean) {  //判断工厂bean
						FactoryBean<?> factory = (FactoryBean<?>) bean;
						boolean isEagerInit;   //是否急切实例化
						if (System.getSecurityManager() != null && factory instanceof SmartFactoryBean) {  //安全管理器不为空，
		...
		for (String beanName : beanNames) {
			...
			//判断单例bean 是否实现了SmartInitializingSingleton
			if (singletonInstance instanceof SmartInitializingSingleton) {
				StartupStep smartInitialize = this.getApplicationStartup().start("spring.beans.smart-initialize")
						.tag("beanName", beanName);

				SmartInitializingSingleton smartSingleton = (SmartInitializingSingleton) singletonInstance; //强转为SmartInitializingSingleton
